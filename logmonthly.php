...
$username = session_check('Location: index.php?page=logmonthly');
...
<html lang='en'>
    <head>
        <title>Expense Manager - Monthly Logs | Jay Creations</title>
        ...
        <script>
            ...
            {
                var bleh;
                for (x=0; x<document.getElementsByClassName('monthlytime').length; x++)
                {
                    bleh = new Date(document.getElementsByClassName('monthlytime')[x].innerHTML.toString());
                    document.getElementsByClassName('monthlytime')[x].innerHTML = bleh.toString().substr(11, 4)+", "+bleh.toString().substr(4, 3);
                }
    ...
    <body class='logbody' onload='initialset()'>
        <div class='container'>
            ...
            <center><h3>Monthly Logs</h3></center>
            <div>
                <center>
...
<?php
                            ...
                            <tr class='trans3inout' title='<?php echo "See more of ".$x[2]; ?>'>
                                <td><a class='monthlytime' href='logdaily.php#<?php echo $x[2]; ?>' id='<?php if ($a != substr($x[2], 0, 4)) echo $a=substr($x[2], 0, 4); ?>'><?php echo $x[2]; ?></a></td>
