...
if (isset($_POST['balsubmit']))
{
    ...
    $query3 = "select `current_balance`, `transaction_time` from `logs` where `transaction_time` < '$time' and `accounts_id` = (select `id` from `accounts` where `name` = '{$_SESSION['accname']}' and `user_id` = (select `id` from `users` where `username` = '$username')) ORDER BY `transaction_time` DESC limit 1";    
...
//    if (!@$con = mysqli_connect("fdb21.awardspace.net", "2759046_em", "Jitin@8943432729", "2759046_em"))
    ...
    if (!$qres=mysqli_query($con, $query3))
        die ("server processes in error");//mysqli_error($con);
    else 
    {
        $row=mysqli_fetch_array($qres);//die("row : ".print_r($row)."<br>old bal : ".$newbal);
        $newbal += $row[0];//die ($newbal);
    }
    ...
    if (!mysqli_query($con, $query))
        die ("cannot execute server processes");//mysqli_error($con));
    if (mysqli_affected_rows($con)>1)
        die ("FATAL ERROR IN LOGGING MECHANISM");//###################################################need an email service to report errors.
    if (!mysqli_query($con, $query2))
        die("Can't execute server processes");//mysqli_error($con));
    mysqli_close($con);
...
<html lang='en'>
    <head>
        ...
        <script type='text/javascript'>
            ...
            {
                var t = new Date();
                var tstring = t.getFullYear()+"-"+('0'+(t.getMonth()+1)).slice(-2)+"-"+('0'+t.getDate()).slice(-2)+"T"+('0'+t.getHours()).slice(-2)+":"+('0'+t.getMinutes()).slice(-2)+":"+'00';
    ...
    <body class='logbody' onload='initialset()'>
        <div class='container-fluid'>
            <div>
                <center>
<!--
                    <form  name='accountselect' action='logall.php' method='get'>
